...
[Flutter](https://www.flutter.io) is an amazing framework, extreamly user friendly and always a pleasure to use. That being said, there might come a time when you might want to integrate native Android/IOS code with Flutter or simply want to pass messages from Flutter to the native side. 
Whatever the reason maybe, one thing is for sure, Flutter makes it very (...like stupid) easy to accomplish this. 

<br>

Flutter allows you to call platform-specific APIs (that are written in the OS native language) with the use of [Platform Channels](https://flutter.io/platform-channels/#codec). It works in a way similar to that of the postal system. Messages are passed from Flutter to the host operation system (either Android or IOS) where it is recieved and an action is carried out. Once it is complete you can send the success or error result back up to Flutter(A way to think of this is like a server. Think of Flutter being the client and the native side being the host/server. Flutter makes a request and the host processes it and then sends a response). 
...
Now the fun part, time to dive into native platform interations. 
First, there are a few things we must do. Import the services package from Flutter and the asynce package from Dart.
...
Under the class declaration, we will create a ```static``` ```const``` variable called ```platformMehtodChannel```
...
Let us break down what is going on here. We are creating a new variable that is awaiting the completion of ```platformMethodChannel.invokeMethod```. But you might be saying to yourself, "What on earth is that!".
 If we look at it closely, we can see that ```platformMethodChannel``` was defined earlier. It is the name given to the MethodChannel we created. Next there is ```.invokeMethod('changeLife');```. This is calls a method on this channel with the name ```'changeLife'```. ```'changeLife'``` will be used on the native side to identify the call and to send the appopriate response. Please note that the string passed to ```.invokeMethod()``` must match on the native side or it will not work! 
 ...
 ```
 That's what the code should now look like after adding the try and catch statements. It is pretty standard. The only strange addition would be the ```on PlatformException``` before the keyword ```catch```. This is coming from the ```services``` package and the TL;DR explaination is, it is used to handle errors coming from the native side. In this example, we are just printing out the error. 
 
 There is one last thing we should do before moving on to coding the host side, Set State with the result. 
...
variable CHANNEL and set it equal to the name given to the method channel in Flutter. I always recommend going to the dart file and copy and pasting the string to avoid any typos. 
...
public class MainActivity extends FlutterActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
```
Next, we will create a new method channel. This takes two arguemtns, the FlutterView(), and the channel.
```java 
public class MainActivity extends FlutterActivity {
...
```
At the end of the MethodChannel, we will attach ```.setMethodCallHandler()``` which as you guessed handles the call. The ```.setMethodCallHandler()``` takes a new  new ```MethodCallHandler()```. If you are using Android Studio then as you type, you should see it suggested by intelisense. (Thanks Flutter for the amazing plugin ;) ).
...
```
As shown above, we created the ```MethodCallHandler``` then overrided ```onMethodCall```.  ```onMethodCall``` runs when the method is called. It has two arguments, _MethodCall_ and _Result_. The MethodCall has all information regarding the actuall call and _result_ handles what you send to Flutter.
...
That's it! With this few lines of code, Flutter and Android should be talking to eachother. Recompile the app and give it a try. 
...
```
Time to recommpile the app and see if it works. You may notice that I used the current API aswell as the deprecated vibrate API. This was intentional. There might be some users who are still using old versions of Android so it is meant as an easy way to support them. 
If all goes to plan, the result should look something like this:
youtube.com

## Conclusion 
Today we learned the basics of using Flutter to communitcate with native code. Being able to communicate with native API's opens up many doors, maybe you want to use one of your favourite Java/Kotlin libraries with Flutter or simply want to open a link in a webview or vibrate the users device. The possiblities are endless. 

The full source code for this project can be found here: placeholder.com. If you find any decrepenses please send me a message and I will rectifiy them as soon as I can. 

This will be my last post for a while. My exams are starting next week and I need to focus my attension on it for the next few weeks ( I actually gratuated High School yesterday :) )
